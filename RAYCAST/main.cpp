...
void Redraw(Person Player, vector <Object*> Map, char* Screen, string Color)
{
	for (int X = -ScreenWidth / 2; X < ScreenWidth / 2; X++)
		for (int Y = -ScreenHeight / 2; Y < ScreenHeight / 2; Y++)
		{
			Angle angle((Player.FOV.x / ScreenWidth) * X, (Player.FOV.y / ScreenHeight) * Y, 0);
			Ray ray(Player, angle);
			ray.Rotation.Normalize();
			double MinDistance = Player.DrawDistance;
			for (auto it = Map.begin(); it != Map.end(); it++)
				if ((*it)->IntersectionDistance(ray) > 0 && (*it)->IntersectionDistance(ray) * Player.Rotation.CosAngleBetweenVectors(ray.Rotation) <= Player.DrawDistance)
				{
					MinDistance = MinDistance < (*it)->IntersectionDistance(ray) ? MinDistance : (*it)->IntersectionDistance(ray) * Player.Rotation.CosAngleBetweenVectors(ray.Rotation);
				}
			char current = Color[MinDistance * (Color.size() - 1) / Player.DrawDistance];
